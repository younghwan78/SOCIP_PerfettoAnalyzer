...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "analyze":
        return _cmd_analyze(args.trace, args.event_config, args.out)
    if args.command == "render-report":
        return _cmd_render(args.trace, args.event_config, args.out)
    if args.command == "check-trace":
        return _cmd_check(args.trace, args.event_config)
    return 2


def _cmd_analyze(trace: Path, event_config: Path, out_dir: Path) -> int:
...
def _cmd_render(trace: Path, event_config: Path, out_path: Path) -> int:
    config = load_event_config(event_config)
    analysis = analyze_trace(trace, config)
    model = build_report_model(analysis, config)
...
def _cmd_check(trace: Path, event_config: Path) -> int:
    config = load_event_config(event_config)
    analysis = analyze_trace(trace, config)
    model = build_report_model(analysis, config)
